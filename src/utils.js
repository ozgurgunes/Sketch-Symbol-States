...
import { fail, dialog } from '@ozgurgunes/sketch-plugin-ui'
...
export function getSymbols(selection) {
  if (
    ...
    fail('Please select symbol instance(s).')
    throw 'Please select symbol instance(s).'
  }
  if (
    ...
    analytics('Selection Not Same')
    fail('Selected symbols must be same.')
    throw 'Selected symbols must be same.'
  }
...
export function getStates(symbol, error) {
  ...
  if (error && states.length < 1) {
    analytics('No States')
    dialog('There are not any states.')
    throw 'There are not any states.'
  }
